...
이번 게임의 네트워크 ping은 단말4인 재검증이 필요하며, 카타리나 화면상 방향도 실기기 확인 전입니다.
실행 경로·백업·한계·공지 수정 명령은 Recovery/V1180Release.md 와 HANDOFF.md 상단 최신 섹션을 참조하세요.

---

# 1.17.5 / 203 — 진영별 알리스타 및 사망·부활 — 2026-09-25
- 사용자 최신 요청으로 알리스타는 각 스폰 진영(블루0/레드1)에 속합니다. 과거 중립/양 팀 공격 가능/즉시 부활 기록은 이전 동작입니다.
- 아군 대상 선택 및 실제 평타·스킬·소환수 피해, 수신 피해 메시지, 보상 지급을 차단합니다. 상대 팀만 공격·처치할 수 있습니다. 소환수/투사체는 소유자 진영 기준.
- 체력20000·적 챔피언 처치1000골드·직전 스폰 옆 위치를 유지합니다. 참가자/상태창 대상에는 포함하지 않습니다.
- 처치 시 실제 Death 상태로 전환, 원본 HumanSM 사망 애니메이션과 알리스타 death.mp3 재생. 죽은 동안 피해·중복 보상을 차단합니다.
...
# 1.17.4 / 202 — 알리스타 배치 및 서버 재점검 — 2026-09-25
- 사용자 첨부 이미지에서 알리스타가 정글 길목에 배치된 것을 확인. 기존 스폰X+/-28 및 최대20유닛 NavMesh 스냅이 다른 구역으로 이동시키는 원인.
- 넓은 맵은 각 스폰에서X+/-12: (-25,3.05,-37)/(25,3.05,37). 직선 맵은X+/-10, Z-/+4: (-27,0.05,-4)/(27,0.21,4). 각 팀 스폰 원 옆에 대칭 배치.
- 스냅 허용1유닛, NavMesh 가장자리 최소1유닛. 조건 미충족 시 생성을 재시도하며 다른 구역으로 임의 이동하지 않음. 네 위치 모두 완전한 스폰 이동 경로·최소1.71유닛 가장자리 간격 및 지면 위 배치를 확인, 두 맵 상단 렌더링으로 검토. 체력20000·1000골드·중립 공격 판정 유지.
- 실제 소켓으로 로딩 중 방장/게스트 탈주 후 남은 인원에게 시작 신호가 누락되는 문제 재현 후 수정. 시작 신호 중복 방지, 남은 참가자의 초기화 검증과 원래 전체 신원 기반 결과 검증을 병행.
- 막힌 송신 큐가 종료돼도 Windows 수신 스레드가 대기 중이면 접속 중으로 남는 문제를 장애 주입으로 재현. 송신 실패 즉시 탈주 처리, 송신/수신 동시 종료는 한 번만 처리.
...
- GitHub 반영 완료: 소스/정리 커밋5080d43를 작업 브랜치에 push, [Draft PR #4](https://github.com/kimjae134679/PhoneLoL_02/pull/4) 생성. 실기기 확인 전 main 자동 병합 없음. 기존 PhoneLOL의 README/Docs/CURRENT_HANDOFF에도 최신 위치 반영(4e5f190).
- 다음 버전 기본값 변경 후 Unity 재컴파일 완료, 오류0.
...
- Confirmed in the 1.7.3 phone/server logs: peer1046 disconnected during room5010 while peer1042 continued. The exact initiating reason was not logged. Both actual game managers still forcibly closed the connection after80 seconds without input; this has been removed. Do not claim AFK was conclusively the cause of that particular disconnect.
- Confirmed result failure: the server validated the submitted full roster against only currently connected peers, rejected the result after a departure, and the client parsed the one-byte error as a result. Authenticated starting identities are now frozen for settlement. Leavers remain in results, identity/host/tamper validation stays enforced, duplicate settlement remains idempotent. Client rejects invalid result responses and invalid85-byte lookup responses with a critical error.
- Managed departure event61002 preserves the original participant identity and actor reference, marks the player offline for the scoreboard, and updates Eve master if the host leaves. Scoreboard uses its existing offline grayscale and adds [나감]. The surviving host owns disconnected scene actors. New and old clients should not be mixed for this behavior.
- Confirmed battle-disconnect callback and loading failures queue a critical Error log and force the development console enabled/visible on the main thread. Startup server panel remains hidden. Editor captured the Error log; Unity native developer-console getters remain false in Editor, so actual phone visibility must be checked.
- Phone logs showed repeated Teemo projectile callback NullReferenceException and a missing effect-attachment component. The exact null field was not retained in IL2CPP traces. Fixed pool lifecycle weaknesses: full resource path keys; prefab identity check; object-reference leases instead of stale instance-ID reverse mappings; generation-checked delayed release; skip destroyed pooled entries; initialize inactive actor state before reset. Do not claim the exact historical projectile trigger was reproduced.
- Verification:1024 alternating Teemo/nexus actual missile hit and state-machine death/reuse cycles; injected stale reverse-ID mapping; old delayed timer cannot retire a new shot. Teemo/Ezreal/Vayne actual animation and missile hit dispatch each passed at attack-speed settings2.5/10/50/200 with10/40/120/240 hits over240 simulated60Hz frames. Engine frame/animation timing still limits effective throughput; original mode rules and uncapped modified-mode stats preserved.
- Alistar root cause confirmed on the broad map: requested flank locations had no nearby NavMesh and heights3 were below terrain heights4.21/6.18. Sampling up to20 units now finds navigable locations, clamps above terrain, and the actor manager retries missing guards during Battle instead of depending on a one-time nexus update. Broad-map targets approximately(1.35,2.05,-33.30)/(-1.93,2.05,33.50); straight-map targets approximately(-9.10,0.05,-6.10)/(9.08,0.33,6.60). Neutral Monster, HP20000, kill gold1000, immediate revival preserved. Spawned guards have visible renderers and duplicate prevention.
- Both scenes' BottomRight group and anchors shifted26 parent units left, about0.3 of the70*1.22 attack-button width. Scale and collider sizes preserved. Nexus uses the ordinary turret warning effect: visible while a living enemy-controlled champion is inside its actual range, hidden on exit/friendly/death.
- Disposable real-socket tests passed host/guest abrupt departures and intentional leaves across mode20/10/103, correct win/loss/lookup including leavers, idempotence and identity/roster/nonhost rejection. Client consumed all six real departure packets, retained identities and changed Eve master. Both game managers tested at10000 seconds idle without kick.
- Live server updated only after confirming no established connections. Backup: recovery/04_runtime/backups/before-v174-20260925-162945. Original16 accounts and SQLite integrity preserved. All five deployed managed-file hashes match source; local diagnostic endpoint127.0.0.1:29000 responds204. Public-address loopback probe115.88.104.135:10045 was refused; external phone reachability was not certified. No router or firewall changes.
- Reports: Recovery/V174ServerVerification.txt, V174ProtocolVerification.txt, V174CombatVerification.txt, V174AnimationVerification.txt, V174SceneInspection.txt, V174DeploymentVerification.txt and V174ServerBackup.txt.
- Next/final action: detached Automation/BuildV174.cs. Target PhoneLOL-02/Builds/PhoneLOL-1.7.4.apk. Recovery/V174BuildJob.json and Builds/build-result.txt are authoritative for completion. Build issuance is not build completion or phone gameplay verification.
- Preserve independent UnityConnectSettings.asset edit, old APKs/backups and remote connection service. No new installation, path move or public GitHub push. Existing Editor/tools paths retained. Disposable packet fixtures and task scratch scripts are removed after validation; verification source and reports retained.
