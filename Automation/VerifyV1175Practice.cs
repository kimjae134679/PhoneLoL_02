foreach(var old in UnityEngine.Object.FindObjectsByType<UnityEngine.GameObject>(UnityEngine.FindObjectsSortMode.None))if(old.name=="V1175 practice fixture")UnityEngine.Object.DestroyImmediate(old);
...
var root=new UnityEngine.GameObject("V1175 practice fixture");
...
try {
 ...
 foreach(byte team in new byte[]{0,1}) {
  ...
  if(guard.OnAttackDamage(attacker)!=0||guard.OnDamage(attacker,20000,20000,20000)!=0)throw new System.Exception("Friendly direct damage");
  guard.OnApplyDamage(0,20000,20000,0,0,800);
  if((float)guard.m_hp!=20000||(int)attacker.get_m_money()!=gold)throw new System.Exception("Friendly replicated damage/reward");
  ...
  for(int i=0;i<3;i++){
   gold=(int)attacker.get_m_money();guard.OnApplyDamage(0,20000,20000,0,0,800);
   ...
   sm.OnUpdateDeath();
   if((float)guard.m_hp!=20000||!guard.IsLive()||sm.get_m_state()!=StateMachine.OEOIIKMBGAG.Idle||go.transform.position!=home||guard.m_dontDamage)throw new System.Exception("Owner RPC respawn failed");
  }
  report.AppendLine("PASS team="+team+" three deaths: +1000 once, dead/untargetable for1 second, owner RPC revives at home HP20000; death animation and Alistar sound="+UnityEditor.AssetDatabase.GetAssetPath(deathClip));
...
System.IO.File.WriteAllText("../Recovery/V1175PracticeVerification.txt",report.ToString());
