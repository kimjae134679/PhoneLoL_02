...
def run():
    with tempfile.TemporaryDirectory(prefix="phonelol-v170-") as tmp:
        ...
        try:
            ...
            for mode,count in ((101,10),(102,10),(103,10)):
                group=[]
                ...
                (pathlib.Path(__file__).resolve().parents[2]/".git"/f"v1180-room-{mode}.bin").write_bytes(roster)
                for c in group:
                ...
                for p in sorted(peers,key=lambda p:p.visual.slot):
                    ...
                    items=8 if mode in (102,103) else 5
...
if __name__=="__main__":
    report=pathlib.Path(__file__).resolve().parents[2]/"Recovery/V1180ServerVerification.txt"
    capture=io.StringIO()
    try:
    ...
    print("1.18.0 integration checks passed.")
