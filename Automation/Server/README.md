# 1.17.4 안정성 재점검
로딩 중 탈주 시 남은 로딩 완료 인원에게 시작 신호를 한 번 보냅니다. 결과는 원래 명단 신원으로 검증하면서 현재 생존 참가자의 월드 초기화를 요구합니다.
송신 큐 실패는 즉시 연결을 정리하고 탈주를 알립니다. 수신 종료와 경합해도 한 번 처리합니다.
관리형 중계의 대상 목록을 잠금 안에서 확정하고66/67의 매 프레임 디스크 로그는 생략합니다. 오류/단절 기록은 유지합니다.
check_v1174.py: 실제10인 중계,120회 접속, 로딩 중 탈주, 막힌 송신 장애 주입. check_v173.py/check_v174.py도 통과.
운영 배포 근거: Recovery/V1174Deployment.json. 지연 수치는 로컬 검사 결과이며 외부망 지연 보장이 아닙니다.
