foreach(var old in UnityEngine.Object.FindObjectsByType<UnityEngine.GameObject>(UnityEngine.FindObjectsSortMode.None))if(old.name=="V1181 practice fixture")UnityEngine.Object.DestroyImmediate(old);
...
var root=new UnityEngine.GameObject("V1181 practice fixture");
...
var managerView=root.AddComponent<EveView>();managerView.set_isSceneObject(true);awake(managerView);var manager=root.AddComponent<ActorManager>();awake(manager);awake(root.AddComponent<GameObjectPool>());
...
try {
 ...
 foreach(byte team in new byte[]{0,1}) {
  ...
  for(int i=0;i<3;i++){
   ...
   typeof(ActorManager).GetMethod("FixedUpdate",flags).Invoke(manager,null);if(guard.IsLive())throw new System.Exception("Early respawn");
   typeof(PhoneLOLPracticeGuardSM).GetField("respawnAt",flags).SetValue(sm,UnityEngine.Time.time-0.01f);
   typeof(ActorManager).GetMethod("FixedUpdate",flags).Invoke(manager,null);
  ...
  report.AppendLine("PASS team="+team+" three deaths: +1000 once, dead/untargetable for1 second, manager RPC revives at home HP10000; death animation and Alistar sound="+UnityEditor.AssetDatabase.GetAssetPath(deathClip));
...
System.IO.File.WriteAllText("../Recovery/V1181PracticeVerification.txt",report.ToString());
